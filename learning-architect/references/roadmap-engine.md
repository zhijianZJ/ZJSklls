...
Use a `buffer_ratio` of 0.20 by default. Raise it for volatile work schedules, unfamiliar delivery environments, or a milestone with external dependencies; record the evidence and assumption. Never count the buffer as assignable task time, and never solve overload by silently increasing learner capacity. Require `weekly_planned_hours[week] <= weekly_delivery_hours[week]` for every week and `phase_planned_hours <= phase_delivery_hours` for the phase.
