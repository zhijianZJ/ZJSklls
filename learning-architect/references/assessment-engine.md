...
Record task IDs, evidence IDs, evaluator, typed observed behaviors, rubric dimension scores, feedback, verification time, and the next action. An applicable behavior requires at least one existing evidence ID and state `pass` for the assessment gate to pass. A behavior may use `not_applicable` only with state `not_applicable`, no evidence IDs, and an explicit decision-relevant reason. Classify results as `understanding`, `guided`, `independent`, or `transfer`; do not infer a higher result from a weighted average alone.
