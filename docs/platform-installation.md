...
| Tencent WorkBuddy | 原生或兼容 Skill，取决于当前版本 | 通过 Skills 界面导入本地目录 | 选择或点名 ZJSkills；返回职业诊断 |
...
WorkBuddy 的 Skills 界面和导入入口可能随版本变化，优先使用当前客户端明确提供的“导入 Skill”“自定义 Skill”或同类入口，不要猜测内部目录。

1. 选择仓库中的整个 `zjskills` 目录；
2. 在安装清单中确认有 `SKILL.md`、`agents/openai.yaml` 和四个引用文件；
3. 审查宿主显示的文件与权限，不覆盖已有同名 Skill；
4. 启用后新建任务，选择 ZJSkills 并发送与 Codex 相同的职业诊断提示词。

如果当前版本没有目录导入能力，可把仓库作为任务工作目录，并要求 WorkBuddy 在列出目标位置、复制文件和权限后再安装。只有宿主确实把目录注册为 Skill 时才称为原生或兼容 Skill；仅把文件放进当前对话属于手动上下文使用。
...
- [Tencent WorkBuddy：功能与 Skills](https://www.workbuddy.ai/docs/zh/workbuddy/From-Beginner-to-Expert-Guide/Function-Description/Task-Bar)
